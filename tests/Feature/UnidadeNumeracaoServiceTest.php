<?php

namespace Tests\Unit;

use App\Service\Unidade\UnidadeNumeracaoService;
use Tests\TestCase;

class UnidadeNumeracaoServiceTest extends TestCase
{
    private UnidadeNumeracaoService $service;

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = app(UnidadeNumeracaoService::class);
    }

    public function test_sem_prefixo_comeca_com_zero()
    {
        $this->assertSame(
            ['01','02','03','04','11','12','13','14'],
            $this->service->gerarNumeracao('01-04', 2)
        );
    }

    public function test_prefixo_1()
    {
        $this->assertSame(
            ['101','102','103','104','111','112','113','114'],
            $this->service->gerarNumeracao('1/01-04', 2)
        );
    }

    public function test_prefixo_10()
    {
        $this->assertSame(
            ['1001','1002','1003','1004','1011','1012','1013','1014'],
            $this->service->gerarNumeracao('10/01-04', 2)
        );
    }

    public function test_nao_duplica_quando_maior_que_10()
    {
        $resultado = $this->service->gerarNumeracao('01-14', 4);

        $this->assertSame(
            count($resultado),
            count(array_unique($resultado))
        );
    }

    public function test_respeita_fim_da_faixa()
    {
        $resultado = $this->service->gerarNumeracao('01-14', 1);

        $this->assertSame('14', end($resultado));
    }

    public function test_padrao_condominio_real()
    {
        $this->assertSame(
            [
                // 1º pavimento (SEM 114)
                '101','102','103','104','105','106','107','108','109','110','111','112','113',

                // 2º pavimento
                '201','202','203','204','205','206','207','208','209','210','211','212','213','214',

                // 3º pavimento
                '301','302','303','304','305','306','307','308','309','310','311','312','313','314',
            ],
            $this->service->gerarNumeracao('01-14', 3, 'condominio')
        );
    }
}
