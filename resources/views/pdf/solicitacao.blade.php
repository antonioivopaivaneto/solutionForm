...
<body class="font-sans antialiased w-full h-full" onload="window.print()">

    <div class="flex w-full h-full">
        <div class="flex-grow">
            <h1 class="text-2xl font-bold uppercase text-white bg-black p-5">Solicitação - {{ $solicitacao->condominio->nome }}</h1>
            <div class="w-full text-sm text-gray-800 border-r-2 border-l-2 border-b-2 border-black p-4">
                ...
                <div class="text-gray-800 text-sm font-semibold border-b-2">{{ $solicitacao->created_at->format('d/m/Y, H:i') }}</div>
                <h2 class="text-gray-600 text-sm font-bold mb-4 mt-5 uppercase">{{ $solicitacao->assunto }}</h2>
                <div class="text-gray-800 text-sm font-semibold">{{ $solicitacao->solicitacao }}</div>
                <div class="mt-5">
        ...
        <div class="w-40 text-center border-r-2 border-b-2 border-black flex flex-col justify-between">
            <div class="text-sm font-bold uppercase text-white bg-black p-4">
                <span>Nº da OS</span>
                <div>{{ $solicitacao->id }}</div>
            </div>
            <div class="text-center text-sm text-gray-800 p-4">
